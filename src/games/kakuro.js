// src/games/kakuro.js — Dream Kakuro+ (final-ux-4, single-daily, smarter-nav)
// ------------------------------------------------------------------
...
// - Endless modes by difficulty; all puzzles have a unique solution (generator owns solution).
...
let soundOn = true;
const _sfx = (kit.audio && kit.audio.sfx) || {
  select(){},
  place(){},
  clear(){},
  errorSoft(){},
...
const sfx = new Proxy(_sfx, {
  get: (t, k) => (...a) => { if (soundOn) t[k]?.(...a); }
});
...
function buildModel(seed) {
  ...
  for (let r = 0; r < H; r++) for (let c = 0; c < W; c++) {
    ...
    if (cell.b && cell.a) {
      ...
      if (cells.length >= 2) {
        ...
        runs.push({ id, orientation: 'across', sum: cell.a, cells });
        cells.forEach(([rr, cc]) => runIdAt[rr][cc] = { ...runIdAt[rr][cc], a: id });
      }
    }
  }
  // down
  for (let r = 0; r < H; r++) for (let c = 0; c < W; c++) {
    ...
    if (cell.b && cell.d) {
      ...
      if (cells.length >= 2) {
        const id = runs.length;
        runs.push({ id, orientation: 'down', sum: cell.d, cells });
        cells.forEach(([rr, cc]) => runIdAt[rr][cc] = { ...runIdAt[rr][cc], d: id });
      }
    }
  }
  const state = [];
...
function cellCandidates(model, r, c) {
  ...
  for (let i = 1; i < sets.length; i++) {
    const next = new Set();
    sets[i].forEach(v => { if (inter.has(v)) next.add(v); });
    inter = next;
  }
  return inter;
}
...
function solveOne(model, maxSteps = 600000) {
  ...
  function pick() {
    ...
    for (let r = 0; r < m.H; r++) for (let c = 0; c < m.W; c++) {
      const cell = m.state[r][c];
      if (cell.b || cell.value) continue;
      const cand = [...cellCandidates(m, r, c)];
...
// (kept for potential future checks / uploaded puzzles)
 ...
 *
 * Primary path:
 *   - random layout → digits (buildSolvedGrid) → clues (buildCluesFromSolution)
 *   - the generator OWNS the solution; we never need to recompute later.
 *
 * Fallback:
 *   - templateVariant → still has a baked-in solution.
 *
 * Last-ditch:
 *   - curated board without a trusted solution (very rare); peek/strict/hints will
 *     simply refuse to enable because there is no canonical solution.
 */
async function generatePuzzle(spec, seedKey, seededRng = null, opts = {}) {
  // For daily puzzles we pass a seededRng (deterministic).
  // For endless puzzles we just use Math.random (non-deterministic).
  const localRng = seededRng || (() => Math.random());
  const rand = () => localRng();
  ...
  // ---- 1) Fresh puzzle path: layout → solved digits → clues, no solver calls ----
  while (performance.now() - start < maxMs) {
    attempt++;

    const mask = generateLayout(spec, rand);
    const H = mask.length;
    ...
    const solved = buildSolvedGrid(runs, rand, H, W);
    if (!solved) continue;

    const grid = buildCluesFromSolution(mask, solved);

    // We *own* this solution; no need to ever recompute it later.
    return {
  ...
  // ---- 2) Fallback: templateVariant (derived from curated seeds, with baked-in solution) ----
  const diffName =
    spec.W <= 9  ? 'Easy'   :
    spec.W <= 10 ? 'Normal' :
  ...
  if (tv) {
    return {
      ...
      grid: tv.grid,
      solution: tv.solution  // baked-in
    };
  }

  // ---- 3) Last-ditch: raw curated grid with *no* trusted solution ----
  // These are very rare. We mark solution as null so strict/peek/hints know
  // there is no canonical solution available.
...
function createGame(seed, seedKey, meta) {
  ...
  function findFirstWhite(m) {
    for (let r = 0; r < m.H; r++)
      for (let c = 0; c < m.W; c++)
        if (!m.state[r][c].b) return { r, c };
    return null;
  }
  ...
  function doSave() {
    ...
    for (let r = 0; r < model.H; r++) {
      ...
      for (let c = 0; c < model.W; c++) {
        const cell = model.state[r][c];
        row.push(cell.b ? null : { v: cell.value, n: [...cell.notes] });
      }
      grid.push(row);
    }
    storage.set(saveKey, {
  ...
  function applyPlace(r, c, v, record = true, announceCb = null) {
    ...
    if (strictMode && v > 0) {
      if (!ensureSolution()) {
        announceCb?.('Strict mode needs a solution (auto-compute failed).');
        return;
      }
    ...
    if (v === 0) {
      cell.value = 0;
      // notes handled by clearCell when we explicitly want both gone
      sfx.clear();
    } else {
      cell.value = v;
      cell.notes.clear();
   ...
   * ensureSolution()
   *
   * - For generated puzzles: meta.solution is already baked in → returns true immediately.
   * - For uploaded / legacy puzzles with clue grid: tries once to solve and cache.
   * - If that fails: returns false (peek/strict/hints not available for that board).
   */
  function ensureSolution() {
    // 1) Fast path: generator / seed already provided a solution.
    if (Array.isArray(meta.solution) && meta.solution.length) {
      return true;
    }

    // 2) Uploaded / legacy puzzles: try to reconstruct from clues once.
    if (!meta.clues) {
      return false;
    }

    const solved = strongSolveFromSeed({ grid: meta.clues });
    if (solved) {
      meta.solution = solved;
      scheduleSave();
      return true;
    }

    // Still nothing: this board has no verified canonical solution.
    return false;
  }
  ...
  function revealHint(announceCb, snackCb) {
    ...
    if (!ensureSolution()) { snackCb?.('This puzzle has no verified solution; hints are disabled.'); return null; }
...
function createRenderer(api, game, seedTitle, onRelaunch) {
  ...
  const openMenu = (btn, id) => {
    const m = root.querySelector(id); if (!m) return;
    closeMenu();
    btn.setAttribute('aria-expanded','true');
    m.classList.remove('hidden');
    openMenuEl = { btn, menu: m };
    positionMenu(btn, m);
  };
  const closeMenu = () => {
  ...
  const showSnack = (msg, ms = 1500) => {
    if (!msg) return;
    snack.textContent = msg;
    snack.classList.add('show');
    setTimeout(() => snack.classList.remove('show'), ms);
  };
  ...
  function buildKeypadHtml() {
    const keys = [];
    for (let d = 1; d <= 9; d++) keys.push({ t: String(d), act: `digit:${d}` });
  ...
  function updateKeypadCandidates() {
    ...
    const allDigits = root.querySelectorAll(
      '.keypad .k[data-act^="digit:"], .keypad-mobile .k[data-act^="digit:"]'
    );
    ...
    candLabel.textContent = s.b
      ? '—'
      : (s.value ? `fixed: ${s.value}` : (cands.size ? [...cands].sort((a, b) => a - b).join(', ') : 'none'));
  }
  ...
  function updateGuideLabels() {
    ...
    const parts = [];
    if (a != null) parts.push(`Across ${a}`);
    if (d != null) parts.push(`Down ${d}`);
  ...
  on(document, 'pointerdown', (e) => {
    if (!openMenuEl) return;
    if (!openMenuEl.menu.contains(e.target) && !openMenuEl.btn.contains(e.target)) closeMenu();
  }, { capture: true });
  ...
  on(window, 'keydown', (e) => {
    ...
    // run navigation
    if (k === 'Tab') {
      if (!e.shiftKey) game.nextInRun(announce);
      needsPaint = true;
      e.preventDefault();
    ...
    // Hold F to peek (works regardless of strict mode)
    if (k.toLowerCase() === 'f') {
      if (game.ensureSolution()) {
        ...
        showSnack('This puzzle has no verified solution; peek is disabled.');
      }
      e.preventDefault();
      return;
    }
  ...
  function tick() {
    ...
    if (sec !== lastTimeSec) {
      lastTimeSec = sec;
      timeEl.textContent = fmtTime(game.getElapsed());
      needsPaint = true;
    }
  ...
  function render() {
    ...
    const active = (() => {
      const s = game.model.state[cur.r][cur.c];
      const set = new Set();
      if (s.runAcross != null) set.add(s.runAcross);
      if (s.runDown   != null) set.add(s.runDown);
      return set;
    })();
    active.forEach(id => {
    ...
    // selection border
    {
      const x = cur.c * cellSize, y = cur.r * cellSize;
      ctx.lineWidth = Math.max(2, Math.floor(cellSize * 0.06));
      ctx.strokeStyle = brand;
      roundRectStroke(ctx, x + 2, y + 2, cellSize - 4, cellSize - 4, Math.min(6, cellSize * 0.15));
    }
  ...
  async function generateAndRelaunch(diff, opts = { daily: false }) {
    ...
    let aborted = false;
    const off = on(overlay, 'click', (e) => {
      if (e.target.closest('[data-act="cancel"]')) {
        aborted = true; overlay.remove(); showSnack('Cancelled'); off();
      }
    });
    ...
    try {
      ...
      overlay.remove();
      showSnack('Using curated board');
      const fb = curatedForDiff(diff);
      const sol = strongSolveFromSeed(fb) || null;
      const niceTitle = opts.daily
        ...
        if (key === 'strict') {
          if (chk.checked) {
            if (game.ensureSolution()) {
              ...
              chk.checked = false; game.strictMode = false; showSnack('This puzzle has no verified solution — strict mode requires a solution.');
            }
...
export default {
  ...
  launch(api, seedArg, seedKeyArg, metaArg) {
    ...
    // If no seed supplied (first load), generate a fresh endless puzzle
    if (!seedArg) {
      // Minimal placeholder UI while we generate
      el.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;color:var(--ink,#e9ecf1);">
          <div style="text-align:center;opacity:.9;">
            <div style="margin-bottom:8px;font-weight:600;">Generating Kakuro puzzle…</div>
      ...
      const diff = metaArg?.diff || DEFAULT_DIFFICULTY;
      const spec = DIFF_SPECS[diff] || DIFF_SPECS[DEFAULT_DIFFICULTY];

      // Endless: no seeded RNG here → each load is a fresh random puzzle
      const seedKey = `initial-endless-${diff}-${Date.now().toString(36)}`;

      generatePuzzle(spec, seedKey, null, { timeBudgetMs: spec.timeBudgetMs })
        .then(puz => {
          if (!puz) throw new Error('generatePuzzle returned null');
          const seed = {
            id: puz.id,
            title: `${diff} • ${spec.W}×${spec.H}`,
            grid: puz.grid,
          ...
          // Relaunch with a proper seed (normal path below)
          this.launch(api, seed, seed.id, meta);
        })
        .catch(err => {
          console.error('Failed to generate initial puzzle', err);
          el.innerHTML = `<div style="padding:16px;color:var(--ink,#e9ecf1);">Failed to generate puzzle.</div>`;
        });

      // Return a dummy handle; real game will return its own when re-launched
      return { dispose() {}, pause() {}, resume() {} };
    }

    // -------- Normal path: we have a seedArg (generated, daily, upload, etc.) --------
    const base = seedArg;
    const baseClues = base.grid;
    ...
    game.ensureSolution(); // strict/peek ready if solvable
